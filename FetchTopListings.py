...
def get_marketplace_rows_from_URL(URL):
    ...
    returns array of tupes of (string item_name, string game_name, int count, float price_starting_at)
    """
    ...
    rows_xml = soup.find_all(class_="market_listing_row")       # Get rows of soup objects for every item in the page
    rows = []
...
def write_to_top_results_database(rows):
    # Add datetime object to the data tuples.
    rows = [(row[0], row[1], row[2], row[3], datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%dT%h:%m:%s")) for row in rows]
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS top_listings (item_name NVARCHAR(64), game_name NVARCHAR(64) NOT NULL, quantity INTEGER NOT NULL, normal_price DECIMAL(9, 2), during DATETIME NOT NULL);")
    cursor.executemany("INSERT INTO top_listings (item_name, game_name, quantity, normal_price, during) VALUES (?, ?, ?, ?, ?)", rows)
    conn.commit()

if __name__=="__main__":
    # Get tuples
    rows = get_marketplace_rows(50)
    # Print tuples
